...
/**
 * This program uses a given set of weights to determine two subsets exist where
 * the difference between their sum totals is equal to another given weight.
 * 
...
public class BalanceScaleLeftRight {
    ...
    static int[] originalWeights = { 35, 61, 1, 5, 225, 8, 12};
    // set to accommodate originalWeights as well as corresponding negative 
    // weights for each corresponding value
    static int[] myWeights = new int[originalWeights.length*2];
    // these weights will be checked against myWeights 
    static int[] toWeigh   = { 1, 2, 3, 4, 6, 7, 9, 14, 17, 18, 20, 24, 28, 110,
    		111, 115, 62, 24, 202, 203, 204, 205};
    ...
    /**
     * Populate a new weight set using originalWeights, but also generating
     * negative values for each corresponding weight. This will simulate 
     * placing on the left or right of the scale.
     *
     */
    public static void buildTestWeightSet(){

        int lengthOriginalWeightSet = originalWeights.length;

        // configure the new weight set with negatives
        for(int i = 0; i < lengthOriginalWeightSet; i++)
        {
            myWeights[i] = originalWeights[i];
            myWeights[i+lengthOriginalWeightSet] = -originalWeights[i];
        }
    }
